...
define('WP_TEAMWARS_CATEGORY', '_wp_teamwars_category');
define('WP_TEAMWARS_DEFAULTCSS', '_wp_teamwars_defaultcss');
define('WP_TEAMWARS_ACL', '_wp_teamwars_acl');
define('WP_TEAMWARS_URL', esc_url(get_template_directory_uri()).'/addons/team-wars');
define('WP_TEAMWARS_IMPORTDIR', 'import');
define('WP_TEAMWARS_IMPORTPATH',  get_template_directory() . '/addons/team-wars/' . WP_TEAMWARS_IMPORTDIR);
define('WP_TEAMWARS_IMPORTURL', WP_TEAMWARS_URL . '/' . WP_TEAMWARS_IMPORTDIR);
require_once(ABSPATH . 'wp-admin/includes/screen.php');

require ( get_template_directory() . '/addons/team-wars/wp-teamwars-widget.php');
require ( get_template_directory() . '/addons/team-wars/wp-upcoming-matches-widget.php');
require ( get_template_directory() . '/addons/team-wars/wp-other-matches-widget.php');
...
class Arcane_TeamWars {
    ...
    function on_init()
    {
        ...
        add_option(WP_TEAMWARS_CATEGORY, -1);
        add_option(WP_TEAMWARS_DEFAULTCSS, true);
        add_option(WP_TEAMWARS_ACL, array());
    ...
    function acl_get() {
        $acl = get_option(WP_TEAMWARS_ACL);
    ...
    function acl_update($user_id, $data) {
        ...
        update_option(WP_TEAMWARS_ACL, $acl);
    ...
    function on_template_redirect() {
        wp_enqueue_script('jquery-tipsy', WP_TEAMWARS_URL . '/js/tipsy/jquery.tipsy.js', array('jquery'), '0.1.7');
        wp_enqueue_script('wp-cw-public', WP_TEAMWARS_URL . '/js/public.js', array('jquery-tipsy'), '1.5.5');
        wp_enqueue_style('jquery-tipsy', WP_TEAMWARS_URL . '/js/tipsy/tipsy.css', array(), '0.1.7');

    }

    function on_wp_footer() {
        if(get_option(WP_TEAMWARS_DEFAULTCSS)) {
    ...
    function on_load_any() {
        wp_enqueue_style('wp-cw-admin', WP_TEAMWARS_URL . '/css/admin.css', array(), '1.5.5');

        wp_enqueue_script('wp-cw-admin', WP_TEAMWARS_URL . '/js/admin.js', array('jquery'), '1.5.5');
        wp_localize_script('wp-cw-admin',
                'wpCWAdminL10n',
                array(
                    'confirmDeleteMap' => esc_html__('Are you sure you want to delete this map?', 'arcane'),
                    'confirmDeleteGame' => esc_html__('Are you sure you want to delete this game?', 'arcane'),
                    'confirmDeleteTeam' => esc_html__('Are you sure you want to delete this team?', 'arcane'),
                    'confirmDeleteMatch' => esc_html__('Are you sure you want to delete this match?', 'arcane')
                )
            );
    }
        ...
        if(!empty($items)) {
            ...
            switch($action) {
                ...
                case 'export':
                    ...
                    header('Content-Disposition: attachment; filename="wp-teamwars-gamepack-' . date('Y-m-d', $this->current_time_fixed('timestamp')) . '.gz"');
                        ...
                        if(!empty($title)) {
                            ...
                            if($data['icon'] >= 0) {
                                ...
                                if($this->add_game($data)) {
                                    wp_redirect(admin_url('admin.php?page=wp-teamwars-games&add=1'));
                                    exit();
                                } else
                        ...
                        if(!empty($title)) {
                            ...
                            if($attach_id >= 0) {
                                ...
                                if($this->update_game($id, $data) !== false) {
                                    wp_redirect(admin_url('admin.php?page=wp-teamwars-games&update=1'));
                                    exit();
                                } else
                                    $this->add_notice(esc_html__('An error occurred.', 'arcane'), 'error');
                        ...
                        if(!empty($title)) {
                            ...
                            if($attach_id >= 0) {
                                ...
                                if($this->add_map(array('title' => $title, 'screenshot' => $attach_id, 'game_id' => $game_id)) !== false) {
                                    wp_redirect(admin_url(sprintf('admin.php?page=wp-teamwars-games&act=maps&game_id=%d&add=1', $game_id)));
                                    exit();
                                } else
                                    $this->add_notice(esc_html__('An error occurred.', 'arcane'), 'error');
                        ...
                        if(!empty($title)) {
                            ...
                            if($attach_id >= 0) {
                                ...
                                if($this->update_map($id, $update_data) !== false) {
                                    wp_redirect(admin_url(sprintf('admin.php?page=wp-teamwars-games&act=maps&game_id=%d&update=1', $game_id)));
                                    exit();
                                } else
                                    $this->add_notice(esc_html__('An error occurred.', 'arcane'), 'error');
                                ...
                                <td class="title column-title">
                                    ...
                                    <div class="row-actions">
                                        <span class="edit"><a href="<?php echo esc_url(admin_url('admin.php?page=wp-teamwars-games&amp;act=edit&amp;id=' . $item->id)); ?>"><?php esc_html_e('Edit', 'arcane'); ?></a></span> |
                                                ...
                                                <a href="<?php echo wp_nonce_url('admin-post.php?action=wp-teamwars-gamesop&amp;do_action=delete&amp;items[]=' . $item->id . '&amp;_wp_http_referer=' . urlencode($_SERVER['REQUEST_URI']), 'wp-teamwars-gamesop'); ?>"><?php esc_html_e('Delete', 'arcane'); ?></a></span>
                                    </div>
                                </td>
                                ...
                                <td class="title column-title">
                                    ...
                                    <div class="row-actions">
                                        <span class="edit"><a href="<?php echo esc_url(admin_url('admin.php?page=wp-teamwars-games&amp;act=editmap&amp;id=' . $item->id)); ?>"><?php esc_html_e('Edit', 'arcane'); ?></a></span> | <span class="delete">
                                                <a href="<?php echo wp_nonce_url('admin-post.php?action=wp-teamwars-deletemaps&amp;do_action=delete&amp;delete[]=' . $item->id . '&amp;_wp_http_referer=' . urlencode($_SERVER['REQUEST_URI']), 'wp-teamwars-deletemaps'); ?>"><?php esc_html_e('Delete', 'arcane'); ?></a></span>
                                    </div>
                                </td>
                            </tr>
...
<?php
    ...
    function on_admin_post_deletematches()
    {
      //UPDATED TO POSTS
        ...
        $referer = remove_query_arg(array('add', 'update', 'lock', 'unlock', 'delete'), $_REQUEST['_wp_http_referer']);
        ...
        if($_REQUEST['do_action'] == 'delete' || $_REQUEST['do_action2'] == 'delete') {
            ...
            $referer = add_query_arg('delete', $error, $referer);
        }else if($_REQUEST['do_action'] == 'lock' || $_REQUEST['do_action2'] == 'lock') {
            extract($this->extract_args($_REQUEST, array('delete' => array())));

            $error = $this->lock_match($delete);
            $referer = add_query_arg('lock', $error, $referer);
        }else if($_REQUEST['do_action'] == 'unlock' || $_REQUEST['do_action2'] == 'unlock') {
            extract($this->extract_args($_REQUEST, array('delete' => array())));

            $error = $this->unlock_match($delete);
            $referer = add_query_arg('unlock', $error, $referer);
        }
    ...
    function update_match_post($match_id, $tid = 0, $type="team") {
        //UPDATED FOR POSTS
        $id = $match_id;
        $post_category = get_option(WP_TEAMWARS_CATEGORY, -1);
        $postarr = array(
            'post_status' => 'publish',
            'post_content' => '',
            'post_excerpt' => '',
        ...
        if ($post->tournament_participants == "team") {
        ...
        if(empty($post->post_title)) {
            ...
            if(!empty($t1) && !empty($t2))
                $post_title = sprintf(esc_html__('%s Versus %s', 'arcane'), $t1, $t2);
            else
                $post_title = esc_html__('Regular match', 'arcane');
        }
        ...
        <div class="mminfow"><div class="mminfo"><strong>'.$m->title.'</strong>  <i class="fa fa-calendar"></i>  ' . date(get_option('date_format').' '.get_option('time_format'), strtotime($m->date)) . '</div></div>
        <div class="dots"></div>
    ...
    <div class="col-lg-12 col-md-12 block ">
        <div id="score_fin" class="mcscalert">
          Score has been submitted by <a href="#">teamnamea</a>! - <span>Accept the score?</span> <a class="ajaxsubmitscore" href="javascript:void(0);" req="accept_score" mid="'.$match_id.'"><i class="fa fa-check"></i></a> <a class="ajaxsubmitscore" href="javascript:void(0);" req="reject_score" mid="'.$match_id.'"><i class="fa fa-times"></i></a>
        </div>
    </div>
    <div class="col-lg-6 col-md-6 block mdescription">
        <div class="title-wrapper">
            <h3 class="widget-title"><i class="fa fa-bullhorn"></i>'.esc_html__(" Match description", "arcane").'</h3></div>
    ...
    '<div class="col-lg-6 col-md-6 block mmaps block">
        <div class="title-wrapper"><h3 class="widget-title"><i class="fa fa-picture-o"></i>'.esc_html__(" Maps", "arcane").'</h3></div>
        <ul>';
          ...
          $r = $this->get_rounds($match_id);
            ...
            $post_content .= '<div class="mscorew">';
               ...
               if($m->status == 'active'){
                ...
                $round_class = $t1 < $t2 ? 'mlose' : ($t1 > $t2 ? 'mwin' : 'mtie');
                    ...
                    $post_content .= sprintf(esc_html__('%d:%d', 'arcane'), $t1, $t2);
                    $post_content .= '</div>';
            ...
            $post_excerpt .= sprintf(_x('%s vs %s', 'match_excerpt', 'arcane'), $m->team1_title, $m->team2_title) . "\n";
    ...
    */
                ...
                <?php endif; ?>
                    ...
                    <form name="match-editor" id="match-editor" method="post" action="<?php echo esc_attr($_SERVER['REQUEST_URI']); ?>" enctype="multipart/form-data">
                        ...
                        <tr class="form-field form-required">
                            ...
                            <td>
                                <?php if (isset($data['tid']) && ($data['tid'] > 0)) { echo '<div style="display:none;">';} ?>
    ...
    function on_load_manage_matches()
    {
        ...
        wp_enqueue_script('wp-cw-matches', WP_TEAMWARS_URL . '/js/matches.js', array('jquery'), '1.5.5');
        wp_localize_script('wp-cw-matches',
                'wpCWL10n',
                array(
                'plugin_url' => WP_TEAMWARS_URL,
        ...
        if(sizeof($_POST) > 0)
        {
            ...
            switch($act) {
                ...
                case 'add':
                    ...
                    if($match_id) {
                        ...
                        wp_redirect(admin_url('admin.php?page=wp-teamwars-matches&add=1'));
                        exit();
                    } else
                        $this->add_notice(esc_html__('An error occurred.', 'arcane'), 'error');
            ...
            case 'edit':
                    ...
                    wp_redirect(admin_url('admin.php?page=wp-teamwars-matches&act=edit&id=' . $id . '&update=1'));
                    exit();
    ...
    function on_shortcode($atts) {
        ...
        $page_links_text = sprintf( '<span class="displaying-num">' . esc_html__( 'Displaying %s&#8211;%s of %s', 'arcane' ) . '</span><div class="pagination-links">%s',
        ...
        $output .= '<ul class="teamwar-list asd">';
      ...
      $output .= '</ul> <div class="clear"></div>
        ...
        foreach($matches as $index => $match) {
            ...
            elseif($is_playing) :
                ...
                $output .= '<div class="scores ' . $wld_class . '">' . sprintf(esc_html__('%d:%d', 'arcane'), $t1, $t2) . '</div>';
            endif;
...
<script>
  ...
  jQuery(document).ready(function() {
    jQuery('#matches-search-submit').click(function (e) {
      e.preventDefault();
  ...
  function insertParam(key, value) {
                            ...
                            <tr class="iedit<?php if($i % 2 == 0) echo ' alternate'; ?>">
                              <th scope="row" class="check-column"><input type="checkbox" name="delete[]" value="<?php echo esc_attr($item->ID); ?>" /></th>
                                <td class="title column-title">
                                    ...
                                    <div class="row-actions">
                                        <span class="edit">
                                          <a href="<?php echo esc_url(admin_url('admin.php?page=wp-teamwars-matches&amp;act=edit&amp;id=' . $item->ID)); ?>"><?php esc_html_e('Edit', 'arcane'); ?></a></span>|
                                        <span class="delete">
                                          <a href="<?php echo wp_nonce_url('admin-post.php?action=wp-teamwars-deletematches&amp;do_action=delete&amp;delete[]=' . $item->ID . '&amp;_wp_http_referer=' . urlencode($_SERVER['REQUEST_URI']), 'wp-teamwars-deletematches'); ?>"><?php esc_html_e('Delete', 'arcane'); ?></a></span>
                                        <!-- pulze -->
                                ...
                                <td class="date column-date">
                                    ...
                                    <?php echo date(get_option('date_format').' '.get_option('time_format'), strtotime($item->date)); ?>
                                </td>
                                ...
                                <td class="team1 column-team1">
                                    ...
                                    if ($is_user_type == false) {
                                        ...
                                        echo esc_html($item->team1_title);
                                    } else {
                                ...
                                <td class="team2 column-team2">
                                    <?php
                                    if ($is_user_type == false) {
                                        echo esc_html($item->team2_title);
                                    } else {
                                ...
                                <td class="reported column-reported">
                                    <?php if($item->status == 'reported1' or $item->status == 'reported2'){ ?><i class="fa fa-exclamation-triangle"></i>&nbsp;<?php esc_html_e('Reported','arcane'); ?><?php } ?>
                            </td>
                                <td class="match_status column-match_status">
                                ...
                                <td class="match_status column-match_status">
                                    <?php if($item->status == 'submitted1'){
                                          if ($is_user_type == false) {
                                            ...
                                            $user = get_user_by('id', $item->team1);
    ...
    function on_admin_post_settings() {
        ...
        if(isset($_POST['category']))
            update_option(WP_TEAMWARS_CATEGORY, (int)$_POST['category']);

        update_option(WP_TEAMWARS_DEFAULTCSS, isset($_POST['enable_default_styles']));
...
// pulze
            ...
            case 'available':
                ...
                foreach($items as $item) {
                    if(isset($available_games[$item])) {
                        ...
                        $content = $this->_get_file_content(trailingslashit(WP_TEAMWARS_IMPORTPATH) . $r->package);
             ...
             <table class="form-table">
                <tr valign="top">
                    ...
                    <td>
                        ...
                        $selected = get_option(WP_TEAMWARS_CATEGORY, -1);
                ...
                <tr valign="top">
                    ...
                    <td><input type="checkbox" name="enable_default_styles" value="true"<?php checked(get_option(WP_TEAMWARS_DEFAULTCSS), true); ?> /></td>
                </tr>
             </table>
                    ...
                    <tbody>
                        ...
                        <tr<?php if($i % 2 == 0) : ?> class="alternate"<?php endif; ?>>
                            ...
                            <td>
                                ...
                                <ul>
                                    <li><?php echo esc_attr($this->acl_keys[$name]); ?>: <?php echo ($is_allowed) ? esc_html__('Yes', 'arcane') : esc_html__('No', 'arcane'); ?></li>
                                </ul>
                                <?php endforeach; ?>
    ...
    function get_available_games() {
        $content = $this->_get_file_content(trailingslashit(WP_TEAMWARS_IMPORTPATH) . 'import.json');
                    ...
                    ?>
                        ...
                        <li>
                            <label for="game-<?php echo esc_attr($index); ?>">
                                <input type="checkbox" name="items[]" id="game-<?php echo esc_attr($index); ?>" value="<?php echo esc_attr($index); ?>" /> <img src="<?php echo esc_attr(trailingslashit(WP_TEAMWARS_IMPORTURL) . $game->icon); ?>" alt="<?php echo esc_attr($game->title); ?>" /> <?php echo esc_html($game->title); ?>
